...
| `WebTransport.h` | **Only where `WEB_FRONTEND_SUPPORT` is on**, which is the two backends with a radio. `WebResponseBody` with `write` / `finish` for one route's answer, and a `WebTransport` singleton with `isListening` / `hasClients` / `sendText` / `sendBinary`. What is *sent* is not here: the catalog, the letter grid, the UTF-8 widening, the frame rate limit and the comparison against the last frame are [`WebFrontend`](../../firmware/inc/Communication/WebFrontend/WebFrontend.h) and are written once. The header must name no framework type by value, because the firmware includes it and cannot promise the include order a backend's `Arduino.h` needs | Not provided — this part has no radio, and the switch stays off |
