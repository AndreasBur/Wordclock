...
weather_read_config_from_eeprom (void)
{
    ...
    {
        if (weather.lon[0] < '0' || weather.lon[0] > '9' || weather.lat[0] < '0' || weather.lat[0] > '9')
        {
            weather.lon[0] = '\0';
            weather.lat[0] = '\0';
        }
...
weather_query (void)
{
    if (*weather.appid)
    {
        ...
        {
            char cmd[32 + MAX_WEATHER_LON_LEN + MAX_WEATHER_LAT_LEN];

            strcpy (cmd, "weather \"");
            strcat (cmd, weather.appid);
            strcat (cmd, "\",\"");
            strcat (cmd, weather.lon);
            strcat (cmd, "\",\"");
            strcat (cmd, weather.lat);
            strcat (cmd, "\"");
        ...
        {
            char cmd[32 + MAX_WEATHER_APPID_LEN + MAX_WEATHER_CITY_LEN];

            strcpy (cmd, "weather \"");
            strcat (cmd, weather.appid);
            strcat (cmd, "\",\"");
            strcat (cmd, weather.city);
            strcat (cmd, "\"");
