...
timeserver_start_timeserver_request (void)
{
    ...
    {
        char send_cmd_buf[8 + MAX_IPADDR_LEN + 1];

        sprintf (send_cmd_buf, "time \"%s\"", timeserver.timeserver);
...
timeserver_convert_time (struct tm * tmp, uint32_t seconds_since_1900)
{
    ...
    curtime += 3600 * timeserver.timezone;                                  // localtime() on STM32 returns GMT, but we want ME(S)Z
