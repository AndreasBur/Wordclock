 ...
 */
int
stm32_serial_poll (long timeout, int log_error)
{
    long    curtime = millis ();
    unsigned int ch;
...
stm32_flash_image (bool do_flash)
{
    ...
    if (f)
    {
        ...
        if (do_flash)
        {
            ...
            http_send (logbuf);
            sprintf (logbuf, "Pages flashed: %ld<BR>\r\n", pages_written);
            http_send (logbuf);
            sprintf (logbuf, "Bytes flashed: %ld<BR>\r\n", bytes_written);
            http_send (logbuf);
 ...
 */
void
stm32_flash_from_server (const char * host, const char * path, const char * filename, int do_unprotect)
{
    SPIFFS.begin();
